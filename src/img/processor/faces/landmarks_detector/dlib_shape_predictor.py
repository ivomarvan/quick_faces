...
__description__ = '''
    Dlib landmarks detector (predictor in some terminology) as img processor
'''
        ...
        for face_result in faces_results:
            ...
            for face_index, face_rectangle in enumerate(faces):
                landmarks = landmarks_to_points(
                    self._predictor(img.get_work_img_array(), face_rectangle.as_dlib_rectangle()))
                face_landmark_couples.append(
                    FaceLandmarsks(face_result=face_result, landmarks=landmarks, face_index=face_index))
